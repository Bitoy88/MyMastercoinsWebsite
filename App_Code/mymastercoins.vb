...
Public Class mymastercoins
    ...
    Function GetandProcessTx(ByVal LastProcess As String, Optional ByVal RefreshMin As Integer = 15) As String
        If LastProcess = "" Then
        ...
        If DateDiff(DateInterval.Minute, LastProcessed, Now) > RefreshMin Then
            Call (New mymastercoins).GetExodusTrans()
                ...
                If Sequence = 1 Then
                    ...
                    Select Case Val(TransType)
                        Case 0
                            ...
                            Dim CurrencyForSale As Double = GetCurrencyForSale(SenderID, CurrencyID)
                ...
                If Sequence = 1 Then
                    ...
                    Select Case Val(TransType)
                        Case 0
                            ...
                            Dim AmountToSend2 As Double = Val(AmounttoSend) / 100000000
                            Dim Balance As Double = GetAddressBalance(SenderID, CurrencyID)
                            Dim ForSale As Double = (New mymastercoins).GetCurrencyForSale(SenderID, CurrencyID)
                            Dim Pending As Double = (New mymastercoins).GetSellerCoinsPendingPayment(SenderID, BlockNumber)

                            If Balance - (ForSale + Pending) >= AmountToSend2 Then
                                'If Not IsDBNull(Row1.Item("MSCReceiverAddress")) Then
                                'If Receipient <> Row1.Item("MSCReceiverAddress") Then
                                'Override possible multiple receipient in transaction parsing
                                'Receipient = Row1.Item("MSCReceiverAddress")
                            ...
                            Else
                                Remarks = "Sender " + ReferenceAddress + " doesn't have " + AmountToSend2.ToString + " " + Currency + ". Bal:" + Balance.ToString + " Sale:" + ForSale.ToString + " Pend:" + Pending.ToString
                            End If
                        Case 20
                            'Selling Coins for Bitcoins
                            Dim CurrencyID As String = DecodeHexString("Currency ID: ", 8)
                            If CurrencyID = 1 And BlockNumber < MSCBlockStart Then
                                Remarks = "MSC Trading has not yet started"
                            Else
                                Dim Currency As String = (New Bitcoin).GetCurrency(CurrencyID)
                                Dim AmountForSale As String = DecodeHexString("Amount for Sale: ", 16)
                                AmountForSale = (CDbl(AmountForSale) / 100000000).ToString
    ...
    Sub ProcessTransactions()
        Dim SQL As String = "SELECT  * from z_Exodus where IsProcessed=0 order by blocknumber,dTrans,ExodusID "
        Dim MSCBlockStart As Integer = 999999999
        Dim DS As DataSet = (New AWS.DB.ConnectDB).SQLdataset(SQL)
        For Each Row1 In DS.Tables(0).Rows
            Dim ExodusID As Integer = Row1.Item("ExodusID")
            ...
            Else
                If Trim(Row1("bcrawtx")) <> "" Then
                    ...
                    'ReferenceAddress is the Sender address I goofed.
                    Dim SenderAddress As String = ""
                    Dim Value2 As Double = 0
                    Dim InputArray As JArray = obj.Item("inputs")

                    For Each subitem In InputArray
                        Dim Address As String = Trim(subitem.Item("prev_out").Item("addr").ToString)
                        '                       Address = obj.Item("txs").Item(0).Item("inputs").ToString
                        'Greatest value is the sender
                        If Address <> "" And Address <> "1EXoDusjGwvnjZUyKkxZ4UHEf77z6A5S4P" Then
                            If Val(subitem.Item("prev_out").Item("value")) > Value2 Then
                                ReferenceAddress = Address
                                SenderAddress = Address
                                Value2 = Val(subitem.Item("prev_out").Item("value"))
                            End If
                        End If
                    Next
                    ...
                    Select Case Val(TransType)
                        ...
                        Case 20
                            ...
                            Else
                                ...
                                Dim Balance As Double = GetAddressBalance(AddressID, CurrencyID)
                                If Balance >= AmountForSale Then
                                    SQL = "select ExodusID from z_CurrencyforSale where Available>0 and IsNewOffer=1 and AddressID=" & AddressID
                                    Dim DSOffer As DataSet = (New AWS.DB.ConnectDB).SQLdataset(SQL)
                                    If DSOffer.Tables(0).DefaultView.Count <> 0 And AmountForSale > 0 Then
                                        Remarks = "Invalid Offer to Sell: " + AmountForSale.ToString + " " + Currency + ".  Offer to Sell still open. XID:" + DSOffer.Tables(0).Rows(0).Item("ExodusID").ToString
                                    Else
    ...
    End Sub
    Sub GetbeReceipient(ByVal beOut As String, ByVal SenderAddress As String, ByRef ClassAReceipient As String, ByRef ClassAData As String, ByRef ThirdAddress As String)
        Dim OutArray As JArray
...
" and MaxBlockNo>=" + BlockNumber.ToString
    ...
    Function GetExodusTransBE() As String
        ...
        Return Result
    End Function
    Function GetExodusTrans() As String
